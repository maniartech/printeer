...
import {
  ...
  ConfigurationError,
} from "../config/enhanced-config-manager";
...
program
  .name("printeer")
  .description(
    "🎯 Enhanced Web-to-PDF/PNG conversion utility with comprehensive configuration, batch processing, and template management",
  )
  .version(getVersion(), "-v, --version", "Display version number");

// Doctor command (kept in enhanced CLI so `printeer --help` includes diagnostics support)
program
  .command("doctor")
  .description("Run system diagnostics and health checks")
  .option("-v, --verbose", "Show detailed diagnostic information")
  .option("--json", "Output results in JSON format")
  .option("--markdown", "Output full Markdown report")
  ...
  .action(async (options) => {
    try {
      ...
      if (options.markdown) {
        console.log(doctor.formatDiagnosticReport(results));
      } else if (options.json) {
        console.log(doctor.formatDiagnosticReportJson(results));
      } else if (!options.quiet) {
        printEnhancedDoctorSummary(results);
      }

      const hasFailures = results.some(
        (r: DiagnosticResult) => r.status === "fail",
      );
...
program
  ...
  .alias("c")
  .description(
    "Convert web page(s) to PDF/PNG with flexible URL-output pairing",
  )
  .argument("[url]", "URL to convert")
  .argument("[output]", "Output filename")
  .option(
    "-u, --url <url>",
    "URL to convert (can be used multiple times)",
    collect,
    [],
  )
  .option(
    "-o, --output <filename>",
    "Output filename for preceding --url (optional)",
    collect,
    [],
  )
  ...
  .option(
    ...
    'Filename pattern for auto-generated names (e.g., "{title}.pdf", "{domain}_{timestamp}.png")',
  )
  .option(
  ...
  .option(
    "--title-fallback",
    "Use webpage title for filename when --output not specified (default behavior)",
  )
  ...
  .option(
    "-f, --format <format>",
    "Page format (A4, A3, Letter, Legal, custom)",
  )
  .option("--orientation <type>", "Page orientation (portrait/landscape)")
  .option(
    "--margins <margins>",
    'Page margins (e.g., "1in" or "top:1in,right:0.5in")',
  )
  ...
  // Performance
  .option(
    "--block-resources <types>",
    "Block resource types (image,stylesheet,font,script)",
  )
  ...
  .option(
    "--concurrency <num>",
    "Concurrent processes for multiple URLs",
    parseInt,
  )
  ...
  .action(async (url, output, options) => {
    try {
      ...
      if (!options.url || options.url.length === 0) {
        throw new Error(
          "At least one URL must be specified (via argument or --url)",
        );
      }
      ...
      if (options.output && options.output.length > options.url.length) {
        throw new Error(
          "Number of --output options cannot exceed number of --url options",
        );
      }
...
configCmd
  ...
  .option(
    "-t, --template <name>",
    "Configuration template (basic, advanced, professional)",
  )
...
function printEnhancedDoctorSummary(results: DiagnosticResult[]): void {
    ...
    const icon =
      result.status === "pass"
        ...
        : result.status === "warn"
          ...
          : "[x]";
    const message =
      result.status === "pass" ? "OK" : result.message.split("\n")[0];
    console.log(`${icon} ${result.component} — ${message}`);
  }
  ...
  const warnings = results.filter(
    (r: DiagnosticResult) => r.status === "warn",
  ).length;

  if (failures > 0) {
    console.error(
      `\n${failures} failure(s) found. Please review remediation suggestions.`,
    );
  } else if (warnings > 0) {
    console.warn(
      `\n${warnings} warning(s) detected. You can proceed, but review recommendations.`,
    );
  } else {
 ...
 */
async function runEnhancedConvertWithPairing(
  options: CliOptions,
): Promise<void> {
  ...
  if (urlOutputPairs.length === 1) {
    ...
    return await runSingleUrlConversion(
      urlOutputPairs[0],
      options,
      configManager,
    );
...
async function runMultiUrlConversion(
  ...
  try {
    ...
    const batchJobs = await createBatchJobsFromPairs(
      pairs,
      options,
      configManager,
    );
    ...
    if (!options.quiet) {
      batchProcessor.on("job-completed", (job, result) => {
        console.log(
          `✓ Completed: ${job.url} -> ${result.outputFile} (${result.duration}ms)`,
        );
      });
    ...
    const report = await batchProcessor.processBatch(
      batchJobs,
    ...
    throw new Error(
      `Multi-URL conversion failed: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  } finally {
...
async function createBatchJobsFromPairs(
  ...
  for (const pair of pairs) {
    try {
      ...
      // Handle conflicts
      const finalOutput = await handleOutputConflicts(
        outputFilename,
        options,
        existingFilenames,
      );
  ...
  if (jobs.length === 0) {
    throw new Error(
      "No URLs to process - all were skipped due to existing files",
    );
  }
...
async function loadAndPrepareConfig(
  ...
  const resolved = await configManager.loadConfiguration(
    options.config,
    options.env,
  );
  ...
  if (!validation.valid) {
    throw new Error(
      `Invalid configuration: ${validation.formattedErrors.join(", ")}`,
    );
  }
...
async function runBatchProcess(batchFile: string, options: any): Promise<void> {
  ...
  if (options.progress && !options.quiet) {
    batchProcessor.on("job-started", (job, progress) => {
      ...
      if (progress) {
        console.log(
          `Processing job ${progress.currentJobNumber}/${progress.totalJobs}`,
        );
      }
    });
  ...
  const report = await batchProcessor.processBatchFile(
    batchFile,
    batchProcessor.options,
  );
...
async function initializeConfig(options: any): Promise<void> {
  ...
  try {
    ...
    if (!options.overwrite) {
      throw new Error(
        `Configuration file already exists: ${outputPath}. Use --overwrite to replace.`,
      );
    }
  } catch (error) {
 ...
 */
async function exportCliToConfig(
  cliCommand: string,
  options: any,
): Promise<void> {
  try {
    ...
    console.error(
      "✗ Failed to export CLI to configuration:",
      error instanceof Error ? error.message : "Unknown error",
    );
    process.exit(1);
  }
 ...
 */
async function generateCliFromConfig(
  configFile: string,
  options: any,
): Promise<void> {
  try {
    ...
    const cliCommand = converter.generateCliCommandFromJson(
      config,
      options.url,
      options.output,
    );
    ...
    console.error(
      "✗ Failed to generate CLI from configuration:",
      error instanceof Error ? error.message : "Unknown error",
    );
    process.exit(1);
  }
...
async function showTemplate(
  name: string,
  options: { preview?: boolean; variables?: string },
): Promise<void> {
  const templateManager = new TemplateManager();
  ...
  try {
    ...
    if (options.variables) {
      try {
        ...
        console.error(
          '  Use: --variables \'{"key":"value"}\' or --variables @file.json',
        );
        process.exit(1);
      }
    }
