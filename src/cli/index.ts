...
async function loadInteractiveUI() {
  try {
    const { intro, outro, text, select, spinner, isCancel, cancel } =
      await import("@clack/prompts");
    return { intro, outro, text, select, spinner, isCancel, cancel };
  } catch (error) {
    throw new Error(
      "Interactive UI dependencies not available. Install @clack/prompts for interactive mode.",
    );
  }
}
...
async function runInteractiveConvert() {
  if (!isInteractive || isQuiet) {
    console.error(
      "Interactive mode requires a TTY and cannot be used with --quiet",
    );
    process.exit(1);
  }
...
async function runInteractiveDoctor(verbose = false) {
  ...
  for (const group of diagnosticGroups) {
    ...
    try {
      ...
      if (failed > 0) {
        s.stop("");
        console.log(
          `❌ ${group.emoji}  ${group.name}: ${failed} failed, ${warnings} warnings, ${passed} passed`,
        );
      } else if (warnings > 0) {
        s.stop("");
        console.log(
          `⚠️ ${group.emoji}  ${group.name}: ${warnings} warnings, ${passed} passed`,
        );
      } else {
        s.stop("");
        console.log(
          `✓ ${group.emoji}  ${group.name}: All ${passed} checks passed`,
        );
      }

      // Show individual test results
      const groupDetails = groupResults
        .map((result) => {
          const icon =
            result.status === "pass"
              ? "✓"
  ...
  const browserLaunch = allResults.find(
    (r) => r.component === "browser-launch",
  );
  if (browserLaunch?.status === "pass") {
    ...
    try {
      ...
      const outputResults = fullResults.filter((r) =>
        ["print-pdf", "print-png"].includes(r.component),
      );

      if (outputResults.length > 0) {
        allResults.push(...outputResults);

        const outputPassed = outputResults.filter(
          (r) => r.status === "pass",
        ).length;
        const outputFailed = outputResults.filter(
          (r) => r.status === "fail",
        ).length;
        ...
        if (outputFailed > 0) {
          console.log(
            `❌ 🎯  Output Generation: ${outputFailed} failed, ${outputPassed} passed`,
          );
        } else {
          console.log(
            `✓ 🎯  Output Generation: All ${outputPassed} checks passed`,
          );
        }

        const outputDetails = outputResults
          .map((result) => {
  ...
  try {
    ...
    if (json) {
      ...
      const byComponent = new Map(
        results.map((r) => [r.component, r] as const),
      );
...
function printRemediationGuide(results: DiagnosticResult[]) {
  ...
  for (const r of issues) {
    ...
    if (r.component === "browser-availability" && r.status === "fail") {
      console.log(
        `  ${color.cyan("Tip:")} Set a custom Chrome/Chromium path via PUPPETEER_EXECUTABLE_PATH`,
      );
    }
  }
}
...
function detailsOf(r: DiagnosticResult): Record<string, unknown> {
  const d =
    r && typeof r.details === "object" && r.details
...
function formatDoctorStyleSummary(results: DiagnosticResult[]): string {
  ...
  const lines: string[] = [];
  lines.push(
    color.bold("Doctor summary (run with --verbose for more details):"),
  );
  ...
  const line = (
    component: string,
    label: string,
    makeMsg?: (r: DiagnosticResult) => string,
  ) => {
  ...
  line("system-info", "System", (r) => {
    ...
    const node =
      typeof d.nodeVersion === "string" ? d.nodeVersion.replace(/^v/, "v") : "";
    const parts = [os, arch && `(${arch})`, node && `Node ${node}`].filter(
      Boolean,
    ) as string[];
    return parts.join(", ");
  });
  ...
  line("browser-sandbox", "Sandbox", (r) =>
    r.status === "pass"
      ? "OK"
      : r.status === "warn"
        ? "Requires --no-sandbox"
  ...
  line("display-server", "Display server", (r) =>
    r.status === "pass" ? "available" : r.message.replace(/\s+/g, " ").trim(),
  );
  line("font-availability", "Fonts", (r) => {
    const d = detailsOf(r);
    return typeof d.totalFonts === "number"
      ? `${d.totalFonts} found`
      : r.status === "pass"
        ? "OK"
        : r.message;
  });
  line("permissions", "Permissions", (r) =>
    r.status === "pass" ? "OK" : r.message,
  );
  line("resource-availability", "Resources", (r) => {
    const d = detailsOf(r);
    const ramNum =
      typeof d.totalMemoryGB === "number"
        ? Math.round(d.totalMemoryGB)
        : undefined;
    const ram = typeof ramNum === "number" ? `${ramNum}GB RAM` : undefined;
    const cores =
      typeof d.cpuCores === "number" ? `${d.cpuCores} cores` : undefined;
  ...
  line("network-connectivity", "Network", (r) =>
    r.status === "pass" ? "OK" : r.message,
  );
  ...
  if (fails === 0 && warns === 0) {
    ...
    lines.push(
      color.yellow(
        `• ${warns} warning(s). You can proceed, but review recommendations.`,
      ),
    );
  } else {
    lines.push(
      color.red(`• ${fails} failure(s). Please review remediation below.`),
    );
  }
  ...
  .action(async (url, output, _options) => {
    ...
    if (!url && !output && isInteractive && !quiet) {
      try {
        ...
        console.error(
          "Interactive mode not available. Use: printeer <url> <output>",
        );
        program.help();
      }
    }
...
export async function runCLI() {
  try {
    ...
    // Check if it's a direct conversion (URL + output without command prefix)
    const looksLikeUrl = args.length > 0 && /^https?:\/\//i.test(args[0]);
    const isDirectConversion =
      args.length >= 2 &&
      looksLikeUrl &&
      !args[0].startsWith("-") &&
      !args[1].startsWith("-") &&
      !knownCommands.has(args[0]);
